...
public class CatmullClarkSubdivide : MonoBehaviour
{
	public static void SubdivideMesh(ref List<Vector3> origVertices_, ref List<int> triangles_, ref List<int> quads_, ref List<Edge> edges_, ref List<FacePoint> faces_)
	{
		...
		for (int i = 0; i < facePoints.Count; i++)
		{
			...
			for (int j = 0; j < tempTopSide.Count; j++)
			{
				Edge topTarget = tempTopSide[j];
				for (int k = 0; k < tempTopSide.Count; k++)
				{
			...
			for (int j = 0; j < tempRightSide.Count; j++)
			{
				Edge rightTarget = tempRightSide[j];
				for (int k = 0; k < tempRightSide.Count; k++)
				{
			...
			for (int j = 0; j < tempBotSide.Count; j++)
			{
				Edge botTarget = tempBotSide[j];
				
				for (int k = 0; k < tempBotSide.Count; k++)
				{
	...
	static void AddFinalVert(Edge value, ref HashSet<Vector3> hashLookUp, ref List<Vector3> finalVerts)
	{					  
	...
	static void AddFinalVert(Vector3 value, ref HashSet<Vector3> hashLookUp, ref List<Vector3> finalVerts)
	{
